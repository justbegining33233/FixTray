...
function TechProfilePageContent() {
  ...
  const [section, setSection] = useState<TechProfileSection>('profile');
  ...
  useEffect(() => {
    ...
    if (raw === 'profile' || raw === 'contact' || raw === 'links') {
      setSection(raw as TechProfileSection);
      return;
    }
    setSection('profile');
  }, [searchParams]);
  ...
  const initials = useMemo(() => {
    const base = (name || user?.name || 'T').trim();
    const chars = base.split(/\s+/).map((p) => p[0]).join('').slice(0, 2);
    return chars.toUpperCase() || 'T';
  }, [name, user]);
