      ...
      <header className="bg-[#000000] border-b border-[#1f2937] px-5 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-semibold"><FaSyncAlt style={{marginRight:4}} /> {say("Recurring Work Orders")}</h1>
          </div>
          <button
            onClick={() => setShowForm(true)}
            className="bg-[#e5332a] hover:bg-[#c62822] text-white px-4 py-2 rounded-lg font-medium text-sm transition-colors"
          >
            {say("+ New Schedule")}{' '}</button>
        </div>
      </header>

      <main className="max-w-5xl mx-auto px-5 py-8">
        <p className="text-stone-400 text-sm mb-6">
          {say("Set up automatic service reminders for repeat customers. The customer will receive a notification and can confirm before a bay is reserved.")}{' '}</p>

        {/* Create Form */}
        {showForm && (
          <div className="bg-[#000000] border border-[#1f2937] rounded-2xl p-6 mb-8">
            <h2 className="text-lg font-semibold mb-4">{say("New Recurring Schedule")}</h2>
            <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div style={{ position: 'relative' }}>
                <label className="block text-sm text-stone-400 mb-1">{say("Customer *")}</label>
                <input
                  required
                  ...
                  placeholder={say("Search by name, phone, or email...")}
                  ...
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: '#000000', border: '1px solid #1f2937', borderRadius: 8, maxHeight: 200, overflowY: 'auto' }}>
                    {customerResults.map((c) => (
                      <button key={c.id} type="button" onClick={() => {
                        ...
                        <div>{say(c.firstName)} {say(c.lastName)}</div>
                        <div style={{ color: '#6b7280', fontSize: 12 }}>{say(c.email)}{c.phone ? `  ${c.phone}` : ''}</div>
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div>
                <label className="block text-sm text-stone-400 mb-1">{say("Service Title *")}</label>
                <input
                  required
                  value={form.title}
                  onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))}
                  placeholder={say("e.g. Oil Change")}
                  className="w-full bg-[#000000] border border-[#1f2937] rounded-lg px-3 py-2 text-sm text-stone-100 focus:outline-none focus:border-[#e5332a]"
                />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm text-stone-400 mb-1">{say("Description *")}</label>
                <textarea
                  required
                  value={form.issueDescription}
                  onChange={(e) => setForm((f) => ({ ...f, issueDescription: e.target.value }))}
                  placeholder={say("What service will be performed?")}
                  rows={2}
              ...
              <div>
                <label className="block text-sm text-stone-400 mb-1">{say("Frequency *")}</label>
                <select
                ...
                >
                  ...
                  <option value="biweekly">{say("Every 2 Weeks")}</option>
                  <option value="monthly">{say("Monthly")}</option>
                  <option value="quarterly">{say("Quarterly")}</option>
                  <option value="annually">{say("Annually")}</option>
                </select>
              </div>
              <div>
                <label className="block text-sm text-stone-400 mb-1">{say("First Run Date")}</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm text-stone-400 mb-1">{say("Estimated Cost ($)")}</label>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                  ...
                  placeholder={say("e.g. 79.99")}
                  className="w-full bg-[#000000] border border-[#1f2937] rounded-lg px-3 py-2 text-sm text-stone-100 focus:outline-none focus:border-[#e5332a]"
                />
              </div>
              <div>
                <label className="block text-sm text-stone-400 mb-1">{say("Service Location")}</label>
                <select
                  value={form.serviceLocation}
                  onChange={(e) => setForm((f) => ({ ...f, serviceLocation: e.target.value }))}
                  className="w-full bg-[#000000] border border-[#1f2937] rounded-lg px-3 py-2 text-sm text-stone-100 focus:outline-none focus:border-[#e5332a]"
                >
                  <option value="in-shop">{say("In Shop")}</option>
                  <option value="roadside">{say("Roadside")}</option>
                </select>
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm text-stone-400 mb-1">{say("Notes")}</label>
                <input
                  value={form.notes}
                  onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
                  placeholder={say("Optional notes for technician")}
                  className="w-full bg-[#000000] border border-[#1f2937] rounded-lg px-3 py-2 text-sm text-stone-100 focus:outline-none focus:border-[#e5332a]"
                />
              </div>
              <div>
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  ...
                  <div>
                    <div className="text-sm text-stone-200 font-medium">{say("Require customer approval")}</div>
                    <div className="text-xs text-stone-500">
                      {form.requiresApproval
                        ? say("Customer will be notified and must confirm before the job is scheduled.")
                        : say("Job is added to the queue automatically without asking the customer.")}
                    </div>
                  </div>
                </label>
              </div>
              {formError && (
                <div className="md:col-span-2 bg-red-900/30 border border-red-500/40 text-red-300 rounded-lg px-4 py-2 text-sm">{say(formError)}</div>
              )}
              <div className="md:col-span-2 flex gap-3">
                <button
                  type="submit"
                  disabled={saving || !form.customerId || !form.title?.trim() || !form.issueDescription?.trim()}
                ...
                >
                  {saving ? say("Creating...") : say("Create Schedule")}
                </button>
                <button
                  type="button"
        ...
        {/* Schedules List */}
        {loading ? (
          <div className="text-center py-16 text-stone-400">{say("Loading schedules...")}</div>
        ) : schedules.length === 0 ? (
          <div className="text-center py-16">
            <div className="text-5xl mb-4"><FaSyncAlt style={{marginRight:4}} /></div>
            <div className="text-stone-300 font-medium text-lg mb-2">{say("No recurring schedules yet")}</div>
            <div className="text-stone-500 text-sm mb-6">
              {say("Set up recurring work orders for repeat services like oil changes, inspections, or maintenance.")}{' '}</div>
            <button
              onClick={() => setShowForm(true)}
              className="bg-[#e5332a] hover:bg-[#c62822] text-white px-6 py-3 rounded-lg font-medium"
            >
              {say("Create Your First Schedule")}{' '}</button>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-semibold text-stone-100">{say(s.title)}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        s.active ? 'bg-green-500/20 text-green-400' : 'bg-stone-500/20 text-stone-400'
                      }`}>
                        {s.active ? say("Active") : say("Paused")}
                      </span>
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        ...
                        {s.requiresApproval ? <><FaBell style={{marginRight:4}} /> {say("Needs approval")}</> : <><FaBolt style={{marginRight:4}} /> {say("Auto-create")}</>}
                      </span>
                    </div>
                    <div className="text-sm text-stone-400 mb-2">{say(s.issueDescription)}</div>
                    <div className="flex flex-wrap gap-4 text-xs text-stone-500">
                      <span><FaUser style={{marginRight:4}} /> {say(s.customer.firstName)} {say(s.customer.lastName)}</span>
                      {s.vehicle && (
                        <span><FaCar style={{marginRight:4}} /> {say(s.vehicle.year)} {say(s.vehicle.make)} {say(s.vehicle.model)}</span>
                      )}
                      {s.estimatedCost && <span><FaDollarSign style={{marginRight:4}} /> ~${s.estimatedCost.toFixed(2)}</span>}
                      <span><FaCalendarAlt style={{marginRight:4}} /> {say("Next:")}{' '}{formatDate(s.nextRunAt)}</span>
                      {s.lastRunAt && <span><FaStopwatch style={{marginRight:4}} /> {say("Last ran:")}{' '}{formatDate(s.lastRunAt)}</span>}
                    </div>
                  </div>
      ...
      {deleteConfirmId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 16, padding: 32, maxWidth: 420, width: '100%', textAlign: 'center' }}>
            ...
            <h3 style={{ fontSize: 18, fontWeight: 700, color: '#f1f5f9', marginBottom: 8 }}>{say("Delete Recurring Schedule?")}</h3>
            <p style={{ color: '#94a3b8', marginBottom: 24, fontSize: 14 }}>{say("This will not affect existing work orders.")}</p>
