...
export default function ConditionReportsPage() {
  ...
  const fuelBar = (pct: number) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <span style={{ fontSize: 12, color: '#9ca3af' }}>{say(pct)}%</span>
    </div>
  );
  ...
  return (
    <div className="centered-app-page" style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ background: 'rgba(0,0,0,0.3)', padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 10 }}><FaCamera style={{fontSize:26}} /> {say("Condition Reports")}</h1>
          <p style={{ margin: '4px 0 0', color: '#9ca3af', fontSize: 14 }}>{say("Vehicle check-in/out condition logs with damage notes  -  protect your shop from liability")}</p>
        </div>
        <button onClick={() => setShowNew(true)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{say("+ New Report")}</button>
      </div>

      <div style={{ padding: '20px 32px 0', display: 'flex', gap: 8 }}>
        {([['all', say("All Reports")], ['check_in', <><FaKey style={{marginRight:4}} />{say("Check-In")}</>], ['check_out', <><FaCar style={{marginRight:4}} />{say("Check-Out")}</>]] as [string, React.ReactNode][]).map(([v, label]) => (
        ...
        {loading ? <div style={{ color: '#6b7280' }}>{say("Loading...")}</div> :
          filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 80 }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 600, margin: '16px 0 8px' }}>{say("No condition reports yet")}</div>
              <div style={{ color: '#9ca3af', marginBottom: 24 }}>{say("Log vehicle condition at check-in and check-out to protect against damage disputes")}</div>
              <button onClick={() => setShowNew(true)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 28px', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>{say("+ Create First Report")}</button>
            </div>
          ) : (
              ...
              {filtered.map(r => (
                <div key={r.id} onClick={() => setSelected(r)} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 18, cursor: 'pointer' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 15 }}>{r.vehicleDesc || say("Vehicle")}</div>
                    ...
                    <span style={{ background: r.reportType === 'check_in' ? 'rgba(96,165,250,0.2)' : 'rgba(34,197,94,0.2)', color: r.reportType === 'check_in' ? '#ff6b64' : '#22c55e', border: `1px solid ${r.reportType === 'check_in' ? '#e5332a' : '#22c55e'}`, borderRadius: 20, padding: '2px 10px', fontSize: 11, fontWeight: 700 }}>
                      {r.reportType === 'check_in' ? <><FaKey style={{marginRight:4}} />{say("Check-In")}</> : <><FaCar style={{marginRight:4}} />{say("Check-Out")}</>}
                    </span>
                  </div>
                  {r.mileageIn && <div style={{ fontSize: 13, color: '#9ca3af', marginBottom: 6 }}>{say("Mileage:")}{' '}{r.mileageIn.toLocaleString()}</div>}
                  {r.fuelLevelIn !== undefined && r.fuelLevelIn !== null && (
                    <div style={{ marginBottom: 6 }}>
                      <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 4 }}>{say("Fuel Level")}</div>
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: 20 }}>
          <div style={{ background: '#1f2937', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 28, width: 450, maxWidth: '100%' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 18 }}>{say("New Condition Report")}</h3>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Report Type")}</label>
              <select value={form.reportType} onChange={e => setForm(p => ({ ...p, reportType: e.target.value }))}
                ...
                <option value="check_in">{say("Check-In")}</option>
                <option value="check_out">{say("Check-Out")}</option>
              </select>
            </div>

            {[["vehicleDesc", say("Vehicle Description"), say("2022 Honda Civic")], ["mileageIn", say("Mileage"), '42500'], ["workOrderId", say("Work Order ID"), say("WO-101")]].map(([k, label, ph]) => (
              <div key={k} style={{ marginBottom: 14 }}>
                <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say(label)}</label>
                <input value={(form as any)[k]} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} placeholder={say(ph)}
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Fuel Level:")}{' '}{say(form.fuelLevelIn)}%</label>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#6b7280' }}><span>{say("Empty")}</span><span>{say("Half")}</span><span>{say("Full")}</span></div>
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea value={form.damageNotes} onChange={e => setForm(p => ({ ...p, damageNotes: e.target.value }))} rows={3}
                placeholder={say("Note any existing dents, scratches, or damage...")}
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Photos")}</label>
              ...
              <label style={{ display: 'inline-block', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '8px 16px', fontSize: 13, color: '#e5e7eb', cursor: 'pointer' }}>
                {photoUploading ? say("Uploading...") : <><FaCamera style={{marginRight:4}} />{say("Add Photo")}</>}
                <input type="file" accept="image/*" onChange={handlePhotoUpload} style={{ display: 'none' }} disabled={photoUploading} />
              </label>
            </div>

            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={create} disabled={saving} style={{ flex: 1, background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '11px 0', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{saving ? say("Saving...") : say("Save Report")}</button>
