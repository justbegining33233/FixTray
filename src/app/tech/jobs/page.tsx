    ...
    <div style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb', padding: 24 }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 700, margin: '12px 0 8px' }}>
          ...
          {view === 'history' ? 'Job History' : 'Active Jobs'}
        </h1>
        <p style={{ color: '#9aa3b2', marginTop: 0 }}>
          {view === 'history' ? 'Completed work assigned to you.' : 'Open work assigned to you.'}
        </p>
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          <Link href={techJobsHref('active') as any} style={{ padding: '8px 14px', borderRadius: 8, textDecoration: 'none', fontWeight: 700, background: view === 'active' ? '#e5332a' : 'rgba(255,255,255,0.08)', color: '#fff' }}>Active</Link>
          <Link href={techJobsHref('history') as any} style={{ padding: '8px 14px', borderRadius: 8, textDecoration: 'none', fontWeight: 700, background: view === 'history' ? '#e5332a' : 'rgba(255,255,255,0.08)', color: '#fff' }}>History</Link>
        </div>
        {loading ? <div style={{ color: '#9aa3b2' }}>Loading jobs...</div> : null}
        ...
        {!loading && !error && mine.length === 0 ? (
          <div style={{ padding: 32, textAlign: 'center', color: '#9aa3b2', background: 'rgba(0,0,0,0.3)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)' }}>
            {view === 'history' ? 'No completed jobs assigned to you yet.' : 'No open jobs assigned to you yet.'}
          </div>
        ) : (
            ...
            {mine.map((order) => (
              <Link key={order.id} href={`/workorders/${order.id}` as any} style={{ display: 'block', textDecoration: 'none', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 16 }}>
                <div style={{ color: '#e5e7eb', fontWeight: 700 }}>
                  WO-{String(order.id).slice(-8).toUpperCase()} · {order.issueDescription?.symptoms || order.issueDescription || order.serviceType || 'Service'}
                </div>
