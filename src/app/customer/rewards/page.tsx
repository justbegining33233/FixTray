  ...
  return (
    <div style={{minHeight:'100vh', background: 'transparent'}}>
      ...
      <div data-desktop-chrome style={{background:'rgba(0,0,0,0.3)', borderBottom:'1px solid rgba(229,51,42,0.3)', padding:'16px 32px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        <div style={{display:'flex', alignItems:'center', gap:24}}>
          ...
          <div>
            ...
            <div style={{fontSize:12, color:'#9aa3b2'}}>{say("Rewards")}</div>
          </div>
        </div>
      ...
      <div style={{maxWidth:1200, margin:'0 auto', padding:32}}>
        <h1 style={{fontSize:32, fontWeight:700, color:'#e5e7eb', marginBottom:32}}>{say("My Rewards")}</h1>

        {/* Current Points */}
        <div style={{background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24, marginBottom:32}}>
          <h2 style={{fontSize:24, fontWeight:700, color:'#e5e7eb', marginBottom:16}}>{say("Current Balance")}</h2>
          <div style={{display:'flex', alignItems:'center', gap:24}}>
            <div>
              <div style={{fontSize:36, fontWeight:700, color:'#e5332a'}}>{say(loyaltyPoints)}</div>
              <div style={{fontSize:14, color:'#9aa3b2'}}>{say("Loyalty Points")}</div>
            </div>
            <div style={{flex:1}}>
              {(() => {
                ...
                return (
                  <>
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom:8}}>
                      <span style={{fontSize:14, color:'#e5e7eb'}}>{say("Progress to next reward")}</span>
                      <span style={{fontSize:14, color:'#9aa3b2'}}>{say(loyaltyPoints)} / {nextThreshold ?? loyaltyPoints}</span>
                    </div>
            ...
            <div key={reward.id} style={{
              ...
              <div style={{marginBottom:16}}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8}}>
                  <h3 style={{fontSize:20, fontWeight:700, color:'#e5e7eb'}}>{say(reward.name)}</h3>
                  {reward.claimed && (
                    <span style={{padding:'4px 8px', background:'rgba(34,197,94,0.2)', color:'#22c55e', borderRadius:6, fontSize:12, fontWeight:600}}>
                      {say("Claimed")}{' '}</span>
                  )}
                </div>
                <div style={{fontSize:18, color:'#e5332a', fontWeight:600, marginBottom:8}}>{say(reward.value)}</div>
                <div style={{fontSize:14, color:'#9aa3b2', marginBottom:8}}>{say(reward.description)}</div>
                <div style={{fontSize:12, color:'#6b7280'}}>{say("Expires:")}{' '}{say(reward.expires)}</div>
                {reward.progress && (
                  <div style={{marginTop:12}}>
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom:4}}>
                      <span style={{fontSize:12, color:'#9aa3b2'}}>{say("Progress")}</span>
                      <span style={{fontSize:12, color:'#9aa3b2'}}>{say(reward.progress)} / {say(reward.total)}</span>
                    </div>
              ...
              {!reward.claimed && (
                <button
                  ...
                  {reward.earned ? say("Claim Reward") : `${(reward.total ?? 0) - loyaltyPoints > 0 ? `${(reward.total ?? 0) - loyaltyPoints} pts needed` : say("Continue Earning")}`}
                </button>
              )}
            </div>
          ))}
        ...
        {history.length > 0 && (
          <div style={{background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24, marginTop:32}}>
            <h2 style={{fontSize:20, fontWeight:700, color:'#e5e7eb', marginBottom:16}}>{say("Points History")}</h2>
            <div style={{display:'grid', gap:8}}>
              {history.map(entry => (
                <div key={entry.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 16px', background:'rgba(255,255,255,0.04)', borderRadius:8}}>
                  <div>
                    <div style={{fontSize:14, color:'#e5e7eb'}}>{say(entry.description)}</div>
                    <div style={{fontSize:12, color:'#9aa3b2'}}>{say(entry.date)}</div>
                  </div>
                  <div style={{fontSize:16, fontWeight:700, color:'#a855f7'}}>+{say(entry.points)} pts</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {loading && (
          <div style={{textAlign:'center', padding:60, color:'#9aa3b2'}}>{say("Loading rewards...")}</div>
        )}
