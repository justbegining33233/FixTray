  ...
  return (
    <div className="centered-app-page" style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ background: 'rgba(0,0,0,0.3)', padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700 }}><FaGift style={{marginRight:4}} /> {say("Referral Program")}</h1>
          <p style={{ margin: '4px 0 0', color: '#9ca3af', fontSize: 14 }}>{say("Track customer referrals and reward them for bringing in new business")}</p>
        </div>
        <button onClick={() => setShowNew(true)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{say("+ New Referral")}</button>
      </div>

      <div style={{ padding: '24px 32px 0', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        {[{ label: say("Total Referrals"), value: stats.total, icon: '' }, { label: say("Converted"), value: stats.converted, icon: '' }, { label: say("Pending"), value: stats.pending, icon: <FaHourglassHalf style={{marginRight:4}} /> }, { label: say("Rewards Issued"), value: `$${stats.value}`, icon: '' }].map(s => (
        ...
        {loading ? <div style={{ color: '#6b7280' }}>{say("Loading...")}</div> :
          referrals.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 80 }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 600, margin: '16px 0 8px' }}>{say("No referrals yet")}</div>
              <div style={{ color: '#9ca3af', marginBottom: 24 }}>{say("Start tracking customer referrals and rewarding loyal customers who bring in new business")}</div>
              <button onClick={() => setShowNew(true)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 28px', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>{say("+ Create First Referral")}</button>
            </div>
          ) : (
            referrals.map(ref => {
              ...
              return (
                <div key={ref.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '16px 20px', marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 15 }}>{ref.referredName || say("Unknown Referral")}</div>
                      <div style={{ fontSize: 13, color: '#9ca3af', marginTop: 2 }}>
                        {ref.referredEmail || ref.referredPhone || ''}  {new Date(ref.createdAt).toLocaleDateString()}
                      </div>
                      {ref.referrer?.name && <div style={{ fontSize: 12, color: '#a78bfa', marginTop: 4 }}>{say("Referred by:")}{' '}{say(ref.referrer.name)}</div>}
                    </div>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      {ref.rewardValue && <span style={{ color: '#22c55e', fontWeight: 700, fontSize: 15 }}>${say(ref.rewardValue)} reward</span>}
                      <span style={{ background: st.bg, color: st.color, border: `1px solid ${st.color}`, borderRadius: 20, padding: '3px 12px', fontSize: 12, fontWeight: 700, textTransform: 'capitalize' }}>{say(ref.status)}</span>
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: 20 }}>
          <div style={{ background: '#1f2937', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 28, width: 440, maxWidth: '100%' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 18 }}>{say("New Referral")}</h3>
            {[["referredName", say("Referred Customer Name"), say("Jane Smith")], ["referredEmail", say("Email"), say("jane@email.com")], ["referredPhone", say("Phone"), '(555) 123-4567']].map(([k, label, ph]) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              <div>
                <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Reward Type")}</label>
                <select value={form.rewardType} onChange={e => setForm(p => ({ ...p, rewardType: e.target.value }))}
                  ...
                  <option value="discount">{say("$ Discount")}</option>
                  <option value="credit">{say("Account Credit")}</option>
                  <option value="gift_card">{say("Gift Card")}</option>
                  <option value="free_service">{say("Free Service")}</option>
                </select>
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Reward Value ($)")}</label>
            ...
            {formError && (
              <div style={{ marginBottom: 14, background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '8px 14px', color: '#fca5a5', fontSize: 13, fontWeight: 600 }}>{say(formError)}</div>
            )}
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={create} disabled={saving || !form.referredName.trim()} style={{ flex: 1, background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '11px 0', fontSize: 14, fontWeight: 600, cursor: saving || !form.referredName.trim() ? 'not-allowed' : 'pointer', opacity: saving || !form.referredName.trim() ? 0.5 : 1 }}>{saving ? say("Saving...") : say("Create Referral")}</button>
