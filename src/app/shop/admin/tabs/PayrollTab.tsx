  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, gap: 10, flexWrap: 'wrap' }}>
        <div>
          <h2 style={{ color: '#e5e7eb', fontSize: 24, margin: 0 }}>{say("Payroll Report")}</h2>
          <div style={{ color: '#9aa3b2', fontSize: 13, marginTop: 4 }}>{say("Live payroll metrics and export controls for this period.")}</div>
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          >
            <FaSyncAlt /> {loading ? say("Refreshing...") : say("Refresh")}
          </button>
          {employees.length > 0 && (
            <>
              <button
              ...
              >
                <FaFilePdf /> {generatingPDF ? say("Generating...") : say("PDF")}
              </button>
            </>
          )}
        </div>
      ...
      <div style={{ marginBottom: 16, background: 'rgba(0,0,0,0.3)', borderRadius: 10, padding: 12 }}>
        <div style={{ color: '#9aa3b2', fontSize: 12, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}><FaCalendarAlt /> {say("Date Range")}</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <label style={{ color: '#9aa3b2', fontSize: 12, display: 'block', marginBottom: 6 }}>{say("Start Date")}</label>
            <input
              type="date"
          ...
          <div>
            <label style={{ color: '#9aa3b2', fontSize: 12, display: 'block', marginBottom: 6 }}>{say("End Date")}</label>
            <input
              type="date"
      ...
      {employees.length === 0 && (
        <div style={{ textAlign: 'center', padding: 48 }}>
          ...
          <div style={{ color: '#9aa3b2', fontSize: 16 }}>{say("No completed time entries found")}</div>
          <div style={{ color: '#6b7280', fontSize: 13, marginTop: 8 }}>{say("Entries will appear here once employees clock out.")}</div>
        </div>
      )}
          ...
          <div style={{ padding: 14, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
              <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: 10, textAlign: 'center' }}>
                <div style={{ color: '#9aa3b2', fontSize: 11 }}>{say("Total Employees")}</div>
                <div style={{ color: '#e5e7eb', fontSize: 22, fontWeight: 800 }}>{say(totalEmployees)}</div>
              </div>
              <div style={{ background: 'rgba(229,51,42,0.12)', borderRadius: 8, padding: 10, textAlign: 'center' }}>
                <div style={{ color: '#9aa3b2', fontSize: 11 }}>{say("Total Hours")}</div>
              ...
              <div style={{ background: 'rgba(34,197,94,0.12)', borderRadius: 8, padding: 10, textAlign: 'center' }}>
                <div style={{ color: '#9aa3b2', fontSize: 11 }}>{say("Total Payroll")}</div>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', fontSize: 14 }}>
              <thead style={{ background: 'rgba(0,0,0,0.3)' }}>
                <tr>
                  <th style={{ padding: 14, textAlign: 'left', color: '#9aa3b2', fontWeight: 700 }}>{say("Employee")}</th>
                  <th style={{ padding: 14, textAlign: 'left', color: '#9aa3b2', fontWeight: 700 }}>{say("Role")}</th>
                  <th style={{ padding: 14, textAlign: 'center', color: '#9aa3b2', fontWeight: 700 }}>{say("Hours")}</th>
                  <th style={{ padding: 14, textAlign: 'center', color: '#9aa3b2', fontWeight: 700 }}>{say("Rate")}</th>
                  <th style={{ padding: 14, textAlign: 'center', color: '#9aa3b2', fontWeight: 700 }}>{say("Total Pay")}</th>
                </tr>
              </thead>
              <tbody>
                {employees.map((emp: any) => (
                  <tr key={emp.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <td style={{ padding: 14, color: '#e5e7eb', fontWeight: 700 }}>{say(emp.name)}</td>
                    <td style={{ padding: 14, color: '#9aa3b2' }}>{emp.role === 'manager' ? say("Manager") : say("Technician")}</td>
