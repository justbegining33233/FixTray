    ...
    <div style={{ minHeight: '100vh', background: '#000000', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ maxWidth: 480, width: '100%' }}>
        ...
        <div style={{ background: '#1e293b', borderRadius: 16, padding: 32, border: '1px solid #334155' }}>
          <h1 style={{ color: '#fff', fontSize: 24, fontWeight: 700, marginBottom: 8 }}><FaLock style={{marginRight:4}} /> {say("Two-Factor Authentication")}</h1>
          <p style={{ color: '#9ca3af', fontSize: 14, marginBottom: 24 }}>
            {say("Add an extra layer of security to your account using an authenticator app.")}{' '}</p>

          {error && <div style={{ background: '#450a0a', color: '#ef4444', border: '1px solid #7f1d1d', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 14 }}>{say(error)}</div>}
          {success && <div style={{ background: '#052e16', color: '#22c55e', border: '1px solid #16a34a', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 14 }}><FaCheckCircle style={{marginRight:4}} /> {say(success)}</div>}

          {enabled || success ? (
            <div style={{ textAlign: 'center', padding: 20 }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}><FaShieldAlt style={{marginRight:4}} /></div>
              <div style={{ color: '#22c55e', fontWeight: 600, fontSize: 16 }}>{say("2FA is Enabled")}</div>
              <div style={{ color: '#6b7280', fontSize: 14, marginTop: 8 }}>{say("Your account is secured with two-factor authentication.")}</div>
            </div>
          ) : step === 'idle' ? (
            <button onClick={startSetup} disabled={fetching}
              ...
              {fetching ? say("Setting up...") : say("Enable 2FA")}
            </button>
          ) : (
            <>
              {qrCode && (
                <div style={{ textAlign: 'center', marginBottom: 20 }}>
                  <div style={{ color: '#9ca3af', fontSize: 13, marginBottom: 8 }}>{say("Scan this QR code with your authenticator app:")}</div>
                  <div style={{ background: '#fff', display: 'inline-block', padding: 12, borderRadius: 12 }}>
                    <img src={qrCode} alt={say("QR Code")} style={{ width: 200, height: 200 }} />
                  </div>
                </div>
              )}
              {secret && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 4 }}>{say("Or enter this key manually:")}</div>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ color: '#9ca3af', fontSize: 13, display: 'block', marginBottom: 4 }}>{say("Enter 6-digit code from your app")}</label>
              ...
              <button onClick={verify} disabled={fetching || code.length !== 6}
                ...
                {fetching ? say("Verifying...") : say("Verify & Enable")}
              </button>
            </>
          )}
        </div>
