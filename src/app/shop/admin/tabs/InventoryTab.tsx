  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, gap: 10, flexWrap: 'wrap' }}>
        <div>
          <h2 style={{ color: '#e5e7eb', fontSize: 24, margin: 0 }}>{say("Inventory Management")}</h2>
          <div style={{ color: '#9aa3b2', fontSize: 13, marginTop: 4 }}>{say("Track stock, handle approvals, and process purchase orders.")}</div>
        </div>
      ...
      <div style={{ marginBottom: 16, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 10 }}>
        <div style={{ background: 'rgba(229,51,42,0.1)', border: '1px solid rgba(229,51,42,0.25)', borderRadius: 10, padding: 12 }}>
          <div style={{ color: '#9aa3b2', fontSize: 11 }}>{say("Inventory Items")}</div>
          <div style={{ color: '#ff6b64', fontSize: 22, fontWeight: 800 }}>{say(inventoryStock.length)}</div>
        </div>
        <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: 10, padding: 12 }}>
          <div style={{ color: '#9aa3b2', fontSize: 11 }}>{say("Units On Hand")}</div>
          <div style={{ color: '#22c55e', fontSize: 22, fontWeight: 800 }}>{say(totalUnits)}</div>
        </div>
        <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: 10, padding: 12 }}>
          <div style={{ color: '#9aa3b2', fontSize: 11 }}>{say("Low Stock")}</div>
          <div style={{ color: '#f59e0b', fontSize: 22, fontWeight: 800 }}>{say(lowStockCount)}</div>
        </div>
        <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 10, padding: 12 }}>
          <div style={{ color: '#9aa3b2', fontSize: 11 }}>{say("Pending Requests")}</div>
          <div style={{ color: '#ef4444', fontSize: 22, fontWeight: 800 }}>{say(inventoryRequests.length)}</div>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 16, marginBottom: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, gap: 10, flexWrap: 'wrap' }}>
          <h3 style={{ color: '#e5e7eb', fontSize: 18, margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
            <FaClipboardList /> {say("Purchase Orders")}{' '}</h3>
          <button
          ...
          >
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <h3 style={{ color: '#e5e7eb', fontSize: 20, margin: 0, fontWeight: 700 }}>{say("Create Purchase Order")}</h3>
              ...
              <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 12, color: '#fbbf24' }}>
                {say("Customer approval is required before ordering. New POs are marked as Awaiting Approval.")}{' '}</div>

              <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr', marginBottom: 14 }}>
                <input type="text" value={poForm.vendor} onChange={(e) => setPoForm({ ...poForm, vendor: e.target.value })} placeholder={say("Vendor (optional)")} style={{ padding: 10, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: '#e5e7eb' }} />
                <input type="text" value={poForm.itemName} onChange={(e) => setPoForm({ ...poForm, itemName: e.target.value })} placeholder={say("Item name")} style={{ padding: 10, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: '#e5e7eb' }} />
                <input type="number" min={1} value={poForm.quantity} onChange={(e) => setPoForm({ ...poForm, quantity: Number(e.target.value) })} placeholder={say("Quantity")} style={{ padding: 10, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: '#e5e7eb' }} />
                <input type="number" min={0} step="0.01" value={poForm.unitCost} onChange={(e) => setPoForm({ ...poForm, unitCost: Number(e.target.value) })} placeholder={say("Unit cost")} style={{ padding: 10, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: '#e5e7eb' }} />
              </div>
              ...
              <div style={{ maxHeight: 140, overflowY: 'auto', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, background: 'rgba(0,0,0,0.3)', marginBottom: 14 }}>
                {loadingWorkOrders ? (
                  <div style={{ color: '#9aa3b2', fontSize: 12, padding: 10 }}>{say("Loading work orders...")}</div>
                ) : workOrderOptions.length === 0 ? (
                  <div style={{ color: '#9aa3b2', fontSize: 12, padding: 10 }}>{say("No open work orders")}</div>
                ) : (
                  workOrderOptions.map((wo: any) => (
                    ...
                    >
                      {wo.id.slice(-6)} | {say(wo.status)} | {wo.issueDescription?.symptoms || ''}
                    </div>
                  ))
                )}
              </div>
        ...
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, background: 'rgba(0,0,0,0.35)', overflow: 'hidden' }}>
          <div style={{ background: 'linear-gradient(90deg,#38bdf8,#6366f1)', color: 'white', padding: '10px 14px', fontWeight: 700 }}>{say("Existing Purchase Orders")}</div>
          <div style={{ padding: 10, display: 'grid', gap: 10 }}>
            {purchaseOrders.length === 0 ? (
              <div style={{ color: '#9aa3b2', fontSize: 13 }}>{say("No purchase orders yet.")}</div>
            ) : (
              purchaseOrders.map((po: any) => (
                <div key={po.id} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, background: 'rgba(0,0,0,0.25)', padding: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <div style={{ color: '#e5e7eb', fontWeight: 700 }}>{say("PO-")}{po.id.slice(-6)} {po.vendor ? `| ${po.vendor}` : ''}</div>
                  ...
                  <div style={{ color: '#9aa3b2', fontSize: 12, marginTop: 6 }}>{say("Items:")}{' '}{po.items?.length || 0} {say("| Created:")}{' '}{new Date(po.createdAt).toLocaleDateString()}</div>
                </div>
              ))
            )}
          </div>
      ...
      {inventoryRequests.length > 0 && (
        <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 12, padding: 16, marginBottom: 18 }}>
          <h3 style={{ color: '#ef4444', fontSize: 18, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
            <FaExclamationTriangle /> {say("Pending Inventory Requests (")}{say(inventoryRequests.length)})
          </h3>
          <div style={{ display: 'grid', gap: 10 }}>
            {inventoryRequests.map((request: any) => (
              <div key={request.id} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, padding: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <div>
                  <div style={{ color: '#e5e7eb', fontWeight: 700, fontSize: 14 }}>{say(request.itemName)} {say("(x")}{say(request.quantity)})</div>
                  <div style={{ color: '#9aa3b2', fontSize: 12 }}>{say("Requested by:")}{' '}{say(request.requesterName)}</div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  >
                    <FaCheckCircle /> {say("Approve")}{' '}</button>
                  <button
                    onClick={async () => {
                      const token = localStorage.getItem('token');
                      await fetch('/api/shop/inventory-requests', {
      ...
      <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: 14, borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(0,0,0,0.25)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <h3 style={{ color: '#e5e7eb', fontSize: 18, margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
              <FaWarehouse /> {say("Inventory")}{' '}</h3>
            <input
              type="text"
              value={inventorySearch}
              onChange={(e) => setInventorySearch(e.target.value)}
              placeholder={say("Search inventory by item, SKU, category, or location")}
              style={{
                width: '100%',
                maxWidth: 420,
                padding: '9px 12px',
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                <th style={{ padding: 14, textAlign: 'left', color: '#9aa3b2', fontSize: 12, fontWeight: 700 }}>{say("Item Name")}</th>
                <th style={{ padding: 14, textAlign: 'left', color: '#9aa3b2', fontSize: 12, fontWeight: 700 }}>{say("SKU")}</th>
                <th style={{ padding: 14, textAlign: 'left', color: '#9aa3b2', fontSize: 12, fontWeight: 700 }}>{say("Category")}</th>
                <th style={{ padding: 14, textAlign: 'center', color: '#9aa3b2', fontSize: 12, fontWeight: 700 }}>{say("Quantity")}</th>
                <th style={{ padding: 14, textAlign: 'center', color: '#9aa3b2', fontSize: 12, fontWeight: 700 }}>{say("Reorder")}</th>
                <th style={{ padding: 14, textAlign: 'center', color: '#9aa3b2', fontSize: 12, fontWeight: 700 }}>{say("Unit Cost")}</th>
                <th style={{ padding: 14, textAlign: 'left', color: '#9aa3b2', fontSize: 12, fontWeight: 700 }}>{say("Location")}</th>
                <th style={{ padding: 14, textAlign: 'center', color: '#9aa3b2', fontSize: 12, fontWeight: 700 }}>{say("Status")}</th>
              </tr>
            </thead>
            <tbody>
              {filteredInventory.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ padding: 40, textAlign: 'center', color: '#9aa3b2' }}>
                    ...
                    <div>{inventoryStock.length === 0 ? say("No inventory items found") : say("No matching inventory items found")}</div>
                  </td>
                </tr>
              ) : (
                filteredInventory.map((item: any) => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: isLowStock ? 'rgba(239,68,68,0.05)' : 'transparent' }}>
                      <td style={{ padding: 14, color: '#e5e7eb', fontWeight: 600 }}>{say(item.itemName)}</td>
                      <td style={{ padding: 14, color: '#9aa3b2', fontSize: 12 }}>{item.sku || '-'}</td>
                      <td style={{ padding: 14, color: '#9aa3b2', fontSize: 12 }}>{item.category || '-'}</td>
                      <td style={{ padding: 14, textAlign: 'center', color: isLowStock ? '#ef4444' : '#e5e7eb', fontWeight: 700 }}>{say(item.quantity)}</td>
                      <td style={{ padding: 14, textAlign: 'center', color: '#9aa3b2', fontSize: 12 }}>{say(item.reorderPoint)}</td>
                      ...
                      <td style={{ padding: 14, textAlign: 'center' }}>
                        {isLowStock ? (
                          <span style={{ padding: '4px 10px', background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 12, color: '#ef4444', fontSize: 11, fontWeight: 700, display: 'inline-block' }}>{say("LOW STOCK")}</span>
                        ) : (
                          <span style={{ padding: '4px 10px', background: 'rgba(34,197,94,0.2)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 12, color: '#22c55e', fontSize: 11, fontWeight: 700, display: 'inline-block' }}>{say("IN STOCK")}</span>
                        )}
                      </td>
                    </tr>
                  );
