...
export default function SystemSettings() {
  ...
  const toggleRow = (label: string, key: string, description?: string) => (
    <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'rgba(255,255,255,0.04)', borderRadius: 8, marginBottom: 10 }}>
      <div>
        <div style={{ color: '#e5e7eb', fontSize: 14, fontWeight: 500 }}>{say(label)}</div>
        {description && <div style={{ color: '#6b7280', fontSize: 12, marginTop: 2 }}>{say(description)}</div>}
      </div>
  ...
  if (isLoading || !settings) {
    return (
      <div style={{ minHeight: "100vh", background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#e5e7eb', fontSize: 18 }}>
        {isLoading ? say("Loading...") : say("Loading settings...")}
      </div>
    );
  }
  if (!user) return null;
    ...
    <div style={{ minHeight: "100vh", background: 'transparent' }}>
      ...
      <div style={{ background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(107,114,128,0.3)', padding: '20px 32px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <Link href="/admin/home" style={{ color: '#e5332a', textDecoration: 'none', fontSize: 14, fontWeight: 600, marginBottom: 12, display: 'inline-block' }}>{say("Back to Dashboard")}</Link>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#e5e7eb', marginBottom: 4 }}>{say("System Settings")}</h1>
          <p style={{ fontSize: 13, color: '#9aa3b2' }}>{say("Configure platform-wide settings and feature flags")}</p>
        </div>
      </div>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 32px' }}>
        <div style={cardStyle}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: '#e5e7eb', marginBottom: 20 }}>{say("General")}</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              <label style={labelStyle}>{say("Platform Name")}</label>
              <input style={fieldStyle} value={settings.platformName || ''} onChange={e => set('platformName', e.target.value)} />
            </div>
            <div>
              <label style={labelStyle}>{say("Support Email")}</label>
            ...
            <div>
              <label style={labelStyle}>{say("Timezone")}</label>
              <select style={fieldStyle} value={settings.timezone || 'America/New_York'} onChange={e => set('timezone', e.target.value)}>
                <option value="America/New_York">{say("Eastern Time (ET)")}</option>
                <option value="America/Chicago">{say("Central Time (CT)")}</option>
                <option value="America/Denver">{say("Mountain Time (MT)")}</option>
                <option value="America/Los_Angeles">{say("Pacific Time (PT)")}</option>
                <option value="America/Phoenix">{say("Arizona (MST, no DST)")}</option>
                <option value="America/Anchorage">{say("Alaska (AKT)")}</option>
                <option value="Pacific/Honolulu">{say("Hawaii (HST)")}</option>
              </select>
            </div>
            
            <div>
              <label style={labelStyle}>{say("Service Fee (cents per work order)")}</label>
              <input style={fieldStyle} type="number" min={0} value={settings.serviceFee ?? 500} onChange={e => set('serviceFee', Number(e.target.value))} />
              <div style={{ fontSize: 11, color: '#6b7280', marginTop: 4 }}>= ${((settings.serviceFee ?? 500) / 100).toFixed(2)} {say("per work order")}</div>
            </div>
          </div>
        </div>

        <div style={cardStyle}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: '#e5e7eb', marginBottom: 16 }}>{say("Feature Flags")}</h2>
        ...
        {settings.maintenanceMode && (
          <div style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: 10, padding: '14px 18px', marginBottom: 24, color: '#fca5a5', fontSize: 14 }}>
            {say("Warning:")}{' '}<strong>{say("Maintenance mode is ON.")}</strong> {say("Non-admin users will be blocked from logging in.")}{' '}</div>
        )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button onClick={() => setResetConfirm(true)} disabled={resetting} style={{ padding: '11px 22px', background: 'transparent', color: '#9aa3b2', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: resetting ? 'not-allowed' : 'pointer' }}>
            {resetting ? say("Resetting...") : say("Reset to Defaults")}
          </button>
      ...
      {resetConfirm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}}>
          <div style={{background:'#1e2533',borderRadius:14,padding:32,minWidth:320,maxWidth:420,boxShadow:'0 8px 32px rgba(0,0,0,0.5)'}}>
            <h3 style={{fontSize:18,fontWeight:700,color:'#e5e7eb',marginBottom:12}}>{say("Reset to Defaults?")}</h3>
            <p style={{fontSize:14,color:'#9aa3b2',marginBottom:24}}>{say("This will reset all settings to their default values. This action cannot be undone.")}</p>
            <div style={{display:'flex',gap:12}}>
              <button onClick={handleReset} style={{flex:1,padding:'10px 0',background:'#ef4444',color:'#fff',border:'none',borderRadius:8,fontSize:14,fontWeight:700,cursor:'pointer'}}>{say("Reset")}</button>
              <button onClick={()=>setResetConfirm(false)} style={{flex:1,padding:'10px 0',background:'transparent',color:'#9aa3b2',border:'1px solid rgba(255,255,255,0.15)',borderRadius:8,fontSize:14,cursor:'pointer'}}>{say("Cancel")}</button>
            </div>
          </div>
        </div>
      )}
