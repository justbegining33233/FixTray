    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#000000' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <main style={{ flex: 1, padding: 24, maxWidth: 1100, margin: '0 auto', width: '100%' }}>
          <h1 style={{ color: '#e5e7eb', fontSize: 28, fontWeight: 700, marginBottom: 8 }}><FaCamera style={{ marginRight: 8 }} /> {say("Shop Photos")}</h1>
          <p style={{ color: '#9aa3b2', marginBottom: 24 }}>{say("Photos uploaded on jobs for this shop.")}</p>
          {loading && <p style={{ color: '#9aa3b2' }}>{say("Loading photos...")}</p>}
          ...
          {!loading && !error && photos.length === 0 && (
            <div style={{ background: '#111827', border: '1px solid #374151', borderRadius: 12, padding: 32, color: '#9aa3b2' }}>
              {say("No shop photos yet. Technicians add photos from their Photos page while they are on a job.")}{' '}</div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
            {photos.map((photo) => (
              <div key={photo.id} style={{ background: '#000', borderRadius: 8, overflow: 'hidden' }}>
                <img src={photo.url} alt={photo.caption || photo.filename || say("Shop photo")} style={{ width: '100%', height: 160, objectFit: 'cover', display: 'block' }} />
                <div style={{ padding: 8, color: '#e5e7eb', fontSize: 12 }}>{photo.caption || photo.filename || say("Photo")}</div>
              </div>
            ))}
          </div>
        </main>
