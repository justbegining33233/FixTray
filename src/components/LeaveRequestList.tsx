      ...
      {requests.length === 0 ? (
        ...
        <div className="space-y-3">
          {requests.map(request => (
            <div
              key={request.id}
              className="p-4 bg-[#0b1220] rounded-lg border border-[#1e293b] text-[#e5e7eb]"
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
                {role === 'manager' && (
                  <div>
                    <p className="text-sm text-slate-400">{say("Technician")}</p>
                    <p className="font-semibold">{say(request.techName)}</p>
                  </div>
                )}
                <div>
                  <p className="text-sm text-slate-400">{say("Dates")}</p>
                ...
                <div>
                  <p className="text-sm text-slate-400">{say("Type")}</p>
                  <span
                ...
                <div>
                  <p className="text-sm text-slate-400">{say("Status")}</p>
                  <span
                    className={`px-2 py-1 rounded text-xs font-semibold ${
              ...
              {request.reason && (
                <div className="bg-gray-50 p-2 rounded text-sm mb-3">
                  <p className="text-slate-400">{say("Reason:")}</p>
