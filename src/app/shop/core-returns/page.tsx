  ...
  return (
    <div className="centered-app-page" style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ background: 'rgba(0,0,0,0.3)', padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 10 }}><FaRecycle style={{fontSize:26}} /> {say("Core Returns")}</h1>
          <p style={{ margin: '4px 0 0', color: '#9ca3af', fontSize: 14 }}>
            ${pendingValue.toFixed(2)} {say("pending recovery  $")}{recoveredValue.toFixed(2)} credited
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={() => { setShowAdd(true); setForm({}); }} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}><FaPlus /> {say("Add Core")}</button>
        </div>
      </div>
        ...
        {loading ? <div style={{ textAlign: 'center', color: '#6b7280', padding: 64 }}>{say("Loading...")}</div> :
          filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 80 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 600, margin: '16px 0 8px' }}>{say("No cores tracked")}</div>
              <div style={{ color: '#9ca3af', marginBottom: 24 }}>{say("Track alternators, starters, and other rebuildable cores to recover value")}</div>
              <button onClick={() => { setShowAdd(true); setForm({}); }} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 28px', fontSize: 15, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}><FaPlus /> {say("Track First Core")}</button>
            </div>
          ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                    {[say("Part Name"), say("Part #"), say("Vendor"), say("Core Value"), say("Credit"), say("Status"), say("WO Ref"), say("Actions")].map(h => (
                      <th key={h} style={{ textAlign: 'left', padding: '10px 14px', color: '#9ca3af', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.5 }}>{say(h)}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(item => (
                    <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                      <td style={{ padding: '12px 14px', fontWeight: 600 }}>{say(item.partName)}</td>
                      ...
                      <td style={{ padding: '12px 14px' }}>
                        <span style={{ background: statusColor[item.status] + '22', color: statusColor[item.status], border: `1px solid ${statusColor[item.status]}`, borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700, textTransform: 'capitalize' }}>{say(item.status)}</span>
                      </td>
                      <td style={{ padding: '12px 14px', color: '#ff6b64' }}>{item.workOrderId ? `#${item.workOrderId.slice(-6).toUpperCase()}` : ' - '}</td>
                      <td style={{ padding: '12px 14px' }}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          {item.status === 'pending' && <button onClick={() => updateStatus(item.id, 'returned')} style={{ background: 'rgba(96,165,250,0.2)', color: '#ff6b64', border: '1px solid #ff6b64', borderRadius: 5, padding: '4px 10px', fontSize: 11, cursor: 'pointer' }}>{say("Returned")}</button>}
                          {item.status === 'returned' && <button onClick={() => { setCreditAmt(String(item.coreValue)); setCreditModal({ id: item.id, defaultAmt: item.coreValue }); }} style={{ background: 'rgba(34,197,94,0.2)', color: '#22c55e', border: '1px solid #22c55e', borderRadius: 5, padding: '4px 10px', fontSize: 11, cursor: 'pointer' }}>{say("Credited")}</button>}
                          {item.status === 'pending' && <button onClick={() => updateStatus(item.id, 'waived')} style={{ background: 'rgba(107,114,128,0.2)', color: '#9ca3af', border: '1px solid #6b7280', borderRadius: 5, padding: '4px 10px', fontSize: 11, cursor: 'pointer' }}>{say("Waive")}</button>}
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: '#1f2937', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 28, width: 420, maxWidth: '90%' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 18 }}>{say("Track Core Return")}</h3>
            ...
            {[["partName", say("Part Name")], ["partNumber", say("Part Number (optional)")], ['vendor', say("Vendor")], ["coreValue", say("Core Value ($)")], ["workOrderId", say("Work Order ID (optional)")], ['notes', say("Notes")]].map(([k, label]) => (
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={save} disabled={saving} style={{ flex: 1, background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '11px 0', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{saving ? say("Saving...") : say("Add Core")}</button>
      ...
      {creditModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: '#1f2937', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 28, width: 360, maxWidth: '90%' }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 18 }}>{say("Credit Amount Received")}</h3>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              >
                {say("Confirm")}{' '}</button>
              <button onClick={() => setCreditModal(null)} style={{ flex: 1, background: 'transparent', color: '#9ca3af', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '11px 0', fontSize: 14, cursor: 'pointer' }}>{say("Cancel")}</button>
            </div>
          </div>
        </div>
      )}
