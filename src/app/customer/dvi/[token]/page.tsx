...
export default function CustomerDVIPage() {
  ...
  if (error || !inspection) return (
    <div style={{ minHeight: '100vh', background: '#000000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#e5e7eb', textAlign: 'center', padding: 32 }}>
      ...
      <h2 style={{ margin: '0 0 8px', fontSize: 22 }}>{say("Inspection Not Found")}</h2>
      <p style={{ color: '#9ca3af' }}>{error || say("This inspection link may have expired or is invalid.")}</p>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000000', color: '#e5e7eb', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#1f2937', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '20px 24px' }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}><FaSearch style={{marginRight:4}} /> {say("Vehicle Inspection Report")}</h1>
        {inspection.vehicleDesc && (
          <p style={{ margin: '4px 0 0', color: '#9ca3af', fontSize: 14 }}>{say(inspection.vehicleDesc)}{inspection.mileage ? `  ${inspection.mileage.toLocaleString()} miles` : ''}</p>
        )}
      ...
      <div style={{ padding: '24px 16px', maxWidth: 640, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
          <div style={{ background: 'rgba(229,51,42,0.1)', border: '1px solid rgba(229,51,42,0.3)', borderRadius: 10, padding: '12px 16px', textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 800, color: '#e5332a' }}>{say(urgentItems.length)}</div>
            <div style={{ fontSize: 12, color: '#9ca3af' }}>{say("Urgent")}</div>
          </div>
          <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 10, padding: '12px 16px', textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 800, color: '#f59e0b' }}>{say(attentionItems.length)}</div>
            <div style={{ fontSize: 12, color: '#9ca3af' }}>{say("Attention")}</div>
          </div>
          {totalEstimate > 0 && (
            <div style={{ background: 'rgba(96,165,250,0.1)', border: '1px solid rgba(96,165,250,0.3)', borderRadius: 10, padding: '12px 16px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 12, color: '#9ca3af' }}>{say("Est. Repairs")}</div>
            </div>
          )}
        </div>
        ...
        {categories.map(cat => {
          ...
          return (
            <div key={cat} style={{ marginBottom: 20 }}>
              <h3 style={{ margin: '0 0 10px', fontSize: 15, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 1 }}>{say(cat)}</h3>
                ...
                {catItems.map(item => (
                  <div key={item.id} style={{ background: conditionBg[item.condition], border: `1px solid ${conditionColor[item.condition]}33`, borderRadius: 10, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      {item.notes && <div style={{ color: '#9ca3af', fontSize: 13, marginTop: 4 }}>{say(item.notes)}</div>}
                      {item.estimatedCost && item.condition !== 'green' && (
                        <div style={{ color: '#60a5fa', fontSize: 13, marginTop: 4 }}>{say("Est. $")}{item.estimatedCost.toFixed(2)}</div>
                      )}
                    </div>
        ...
        {!inspection.customerApproved && !approved ? (
          <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 12, padding: 20, marginTop: 24, textAlign: 'center' }}>
            <h3 style={{ margin: '0 0 8px', fontSize: 16 }}>{say("Approve Recommended Services")}</h3>
            <p style={{ color: '#9ca3af', fontSize: 14, margin: '0 0 16px' }}>{say("By approving, you authorize the shop to proceed with the recommended repairs.")}</p>
            <button onClick={handleApprove} disabled={approving}
              ...
              {approving ? say("Approving...") : <><FaCheckCircle style={{marginRight:4}} /> {say("Approve Services")}</>}
            </button>
            {error && <div style={{ color: '#ef4444', fontSize: 13, marginTop: 10 }}>{say(error)}</div>}
          </div>
        ) : (
          <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 12, padding: 20, marginTop: 24, textAlign: 'center' }}>
            <div style={{ fontSize: 40 }}><FaCheckCircle style={{marginRight:4}} /></div>
            <h3 style={{ margin: '8px 0 4px', fontSize: 16 }}>{say("Services Approved")}</h3>
            <p style={{ color: '#9ca3af', fontSize: 14, margin: 0 }}>{say("Your shop has been notified. They will proceed with the approved repairs.")}</p>
          </div>
        )}
      </div>
    </div>
