  ...
  return (
    <div className="centered-app-page" style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ background: 'rgba(0,0,0,0.3)', padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700 }}><FaBolt style={{marginRight:4}} /> {say("Automations")}</h1>
          <p style={{ margin: '4px 0 0', color: '#9ca3af', fontSize: 14 }}>{say("Automated SMS/email reminders for appointments, follow-ups, and review requests")}</p>
        </div>
        <button onClick={() => { setEditing(null); setForm({ name: '', type: 'appointment_reminder', trigger: 'days_before_appointment', triggerValue: '1', channel: 'sms', messageTemplate: '', isActive: true }); setShowForm(true); }}
          style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{say("+ New Rule")}</button>
      </div>
        ...
        {loading ? <div style={{ color: '#6b7280' }}>{say("Loading...")}</div> :
          rules.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 80 }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 600, margin: '16px 0 8px' }}>{say("No automation rules yet")}</div>
              <div style={{ color: '#9ca3af', marginBottom: 24 }}>{say("Set up automated reminders and follow-ups to save time and increase repeat business")}</div>
              <button onClick={() => setShowForm(true)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 28px', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>{say("+ Create First Automation")}</button>
            </div>
          ) : (
              ...
              {rules.map(rule => (
                <div key={rule.id} style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${rule.isActive ? 'rgba(34,197,94,0.3)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 12, padding: 18 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 15 }}>{say(rule.name)}</div>
                    ...
                    <button onClick={() => toggleActive(rule)} style={{ background: rule.isActive ? 'rgba(34,197,94,0.2)' : 'rgba(107,114,128,0.2)', color: rule.isActive ? '#22c55e' : '#6b7280', border: `1px solid ${rule.isActive ? '#22c55e' : '#6b7280'}`, borderRadius: 20, padding: '3px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                      {rule.isActive ? say("Active") : say("Paused")}
                    </button>
                  </div>
                  <div style={{ fontSize: 13, background: 'rgba(255,255,255,0.05)', borderRadius: 8, padding: '10px 14px', marginBottom: 10, color: '#d1d5db', fontStyle: 'italic' }}>
                    {say("&ldquo;")}{rule.messageTemplate.slice(0, 100)}{rule.messageTemplate.length > 100 ? '...' : ''}{say("&rdquo;")}{' '}</div>
                  <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 10 }}>
                     {rule.channel.toUpperCase()}  {say("Trigger:")}{' '}{TRIGGER_OPTIONS.find(t => t.value === rule.trigger)?.label || rule.trigger} {rule.triggerValue ? `(${rule.triggerValue})` : ''}
                    {rule._count && <span style={{ marginLeft: 12 }}> {say(rule._count.executions)} sent</span>}
                  </div>
                  {!!rule.executions?.length && (
                    <div style={{ marginBottom: 10, background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: '8px 10px' }}>
                      <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 6 }}>{say("Recent Executions")}</div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => openEdit(rule)} style={{ flex: 1, background: 'rgba(255,255,255,0.08)', color: '#e5e7eb', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 7, padding: '6px 0', fontSize: 13, cursor: 'pointer' }}>{say("Edit")}</button>
                    <button onClick={() => setDeleteConfirmId(rule.id)} style={{ background: 'rgba(229,51,42,0.15)', color: '#e5332a', border: '1px solid rgba(229,51,42,0.3)', borderRadius: 7, padding: '6px 14px', fontSize: 13, cursor: 'pointer' }}>{say("Delete")}</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, overflowY: 'auto', padding: 20 }}>
          <form onSubmit={(e) => { e.preventDefault(); save(); }} style={{ background: '#1f2937', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 28, width: 520, maxWidth: '100%' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 18 }}>{editing ? say("Edit Automation") : say("New Automation Rule")}</h3>

            {[{ k: 'name', label: say("Rule Name *"), ph: say("Appointment Reminder 1 Day Before") }].map(({ k, label, ph }) => (
              <div key={k} style={{ marginBottom: 14 }}>
                <label htmlFor="automation-rule-name" style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say(label)}</label>
                <input id="automation-rule-name" name="name" required aria-required="true" value={(form as any)[k]} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} placeholder={say(ph)}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
              <div>
                ...
                <select value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))}
                  ...
                  {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{say(v)}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Channel")}</label>
                <select value={form.channel} onChange={e => setForm(p => ({ ...p, channel: e.target.value }))}
                  style={{ width: '100%', background: '#374151', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '10px 12px', color: '#e5e7eb', fontSize: 14 }}>
                  <option value="sms"><FaMobileAlt style={{marginRight:4}} /> {say("SMS")}</option>
                  <option value="email"><FaEnvelope style={{marginRight:4}} /> {say("Email")}</option>
                  <option value="both"><FaMobileAlt style={{marginRight:4}} /><FaEnvelope style={{marginRight:4}} /> {say("Both")}</option>
                </select>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, marginBottom: 14 }}>
              <div>
                <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Trigger")}</label>
                <select value={form.trigger} onChange={e => setForm(p => ({ ...p, trigger: e.target.value }))}
                  style={{ width: '100%', background: '#374151', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '10px 12px', color: '#e5e7eb', fontSize: 14 }}>
                  {TRIGGER_OPTIONS.map(t => <option key={t.value} value={t.value}>{say(t.label)}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Value")}</label>
            ...
            <div style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <label htmlFor="automation-message-template" style={{ fontSize: 13, color: '#9ca3af' }}>{say("Message Template *")}</label>
                <span style={{ fontSize: 11, color: '#6b7280' }}>{say("Click to insert variable")}</span>
              </div>
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 8 }}>
                {VARIABLE_HINTS.map(v => (
                  <button type="button" key={v} onClick={() => insertVar(v)} style={{ background: 'rgba(229,51,42,0.15)', color: '#ff6b64', border: '1px solid rgba(229,51,42,0.3)', borderRadius: 5, padding: '3px 8px', fontSize: 11, cursor: 'pointer' }}>{say(v)}</button>
                ))}
              </div>
              <textarea id="automation-message-template" name="messageTemplate" required aria-required="true" value={form.messageTemplate} onChange={e => setForm(p => ({ ...p, messageTemplate: e.target.value }))} rows={4}
                placeholder={say("Hi {customer_name}, your appointment for your {vehicle} is tomorrow at {appointment_time}. Reply CONFIRM or call us at (555) 123-4567.")}
            ...
            {formError && <div style={{ background: 'rgba(239,68,68,0.15)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '10px 14px', marginBottom: 12, fontSize: 13 }}>{say(formError)}</div>}
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button type="submit" disabled={saving || !form.name.trim() || !form.messageTemplate.trim()} style={{ flex: 1, background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '11px 0', fontSize: 14, fontWeight: 600, cursor: saving || !form.name.trim() || !form.messageTemplate.trim() ? 'not-allowed' : 'pointer', opacity: saving || !form.name.trim() || !form.messageTemplate.trim() ? 0.5 : 1 }}>{saving ? say("Saving...") : editing ? say("Save Changes") : say("Create Rule")}</button>
              <button type="button" onClick={() => { setShowForm(false); setEditing(null); setFormError(''); }} style={{ flex: 1, background: 'transparent', color: '#9ca3af', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '11px 0', fontSize: 14, cursor: 'pointer' }}>{say("Cancel")}</button>
            </div>
          </form>
        </div>
      )}
      ...
      {deleteConfirmId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#1f2937', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 28, maxWidth: 360, width: '90%', textAlign: 'center' }}>
            <h3 style={{ color: '#f1f5f9', fontSize: 18, fontWeight: 700, marginBottom: 12 }}>{say("Delete Rule?")}</h3>
            <p style={{ color: '#9ca3af', marginBottom: 24 }}>{say("This automation rule will be permanently deleted.")}</p>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              <button onClick={() => deleteRule(deleteConfirmId)} style={{ background: '#e5332a', border: 'none', color: '#fff', padding: '10px 24px', borderRadius: 8, fontWeight: 600, cursor: 'pointer' }}>{say("Delete")}</button>
