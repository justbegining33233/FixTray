          ...
          <>
            ...
            {showAddForm && (
              <div style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 12, padding: 24, marginBottom: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
                  {[
                    { label: say("Name"), key: 'name', type: 'text' },
                    { label: say("SKU"), key: 'sku', type: 'text' },
                    { label: say("Category"), key: 'category', type: 'text' },
                    { label: say("Location"), key: 'location', type: 'text' },
                    { label: say("Quantity"), key: 'quantity', type: 'number' },
                    { label: say("Reorder Point"), key: "reorderPoint", type: 'number' },
                    { label: say("Unit Cost ($)"), key: "unitCost", type: 'number' },
                    { label: say("Selling Price ($)"), key: "sellingPrice", type: 'number' },
                  ].map(f => (
                    <div key={f.key}>
                ...
                <div style={{ marginTop: 12 }}>
                  <label style={{ display: 'block', fontSize: 11, color: '#9aa3b2', marginBottom: 4 }}>{say("Description")}</label>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                      {[say("Name"), say("SKU"), say("Category"), say("Qty"), say("Reorder"), say("Cost"), say("Price"), say("Actions")].map(h => (
                  ...
                  <tbody>
                    {filteredItems.length === 0 ? (
                      ...
                      <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <td style={{ padding: '12px 16px', fontSize: 13, fontWeight: 700, color: item.quantity <= item.reorderPoint ? '#ef4444' : '#22c55e' }}>{say(item.quantity)}</td>
                        <td style={{ padding: '12px 16px', fontSize: 12, color: '#9aa3b2' }}>{say(item.reorderPoint)}</td>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16, marginTop: 24 }}>
              {[
                ...
                { label: say("Low Stock"), value: items.filter(i => i.quantity <= i.reorderPoint).length, color: '#ef4444' },
                { label: say("Total Value"), value: `$${items.reduce((sum, i) => sum + (i.quantity * i.unitCost), 0).toFixed(2)}`, color: '#10b981' },
              ].map(stat => (
