  ...
  return (
    <div className="centered-app-page" style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb', fontFamily: 'system-ui,sans-serif' }}>
      ...
      <div style={{ background: 'rgba(0,0,0,0.3)', padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <button onClick={() => router.back()} style={{ background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: 13, marginBottom: 6, padding: 0 }}><FaArrowLeft style={{marginRight:4}} /> {say("Back")}</button>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700 }}><FaShoppingCart style={{marginRight:4}} /> {say("Purchase Orders")}</h1>
          <p style={{ margin: '4px 0 0', color: '#9ca3af', fontSize: 14 }}>{say("Manage vendor purchase orders and parts procurement")}</p>
        </div>
      ...
      <div style={{ padding: 32 }}>
        ...
        {error && (
          <div style={{ background: 'rgba(229,51,42,0.1)', border: '1px solid rgba(229,51,42,0.3)', borderRadius: 10, padding: '14px 20px', marginBottom: 24, display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <button onClick={load} style={{ marginLeft: 'auto', background: '#e5332a', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 14px', cursor: 'pointer', fontSize: 13 }}>{say("Retry")}</button>
          </div>
        )}

        {/* Loading */}
        {loading && <div style={{ textAlign: 'center', padding: 60, color: '#6b7280' }}>{say("Loading purchase orders...")}</div>}
          ...
          <div style={{ textAlign: 'center', padding: '60px 0', background: 'rgba(255,255,255,0.03)', borderRadius: 14, border: '1px dashed rgba(255,255,255,0.12)' }}>
            ...
            {statusFilter === 'all' && (
              <button onClick={() => setShowNew(true)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                {say("+ Create Purchase Order")}{' '}</button>
            )}
          </div>
        )}
              ...
              return (
                <div key={order.id} onClick={() => setSelected(order)} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 12, padding: '18px 24px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    <div style={{ fontWeight: 700, fontSize: 15 }}>{say(order.poNumber)}</div>
                    <div style={{ color: '#9ca3af', fontSize: 13, marginTop: 2 }}><FaIndustry style={{marginRight:4}} /> {say(order.vendor)}</div>
                  </div>
                  <div style={{ color: '#6b7280', fontSize: 13 }}>
                    {order.items?.length ?? 0} item{(order.items?.length ?? 0) !== 1 ? 's' : ''}
                  </div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: '#22c55e' }}>${(order.total ?? 0).toFixed(2)}</div>
                  <div style={{ padding: '4px 10px', borderRadius: 16, background: s?.bg, color: s?.color, fontSize: 12, fontWeight: 700 }}>{say(s?.label)}</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: '#1a1d27', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 16, padding: 32, width: '100%', maxWidth: 560, maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>{say(selected.poNumber)}</h2>
                <div style={{ color: '#9ca3af', fontSize: 13, marginTop: 4 }}><FaIndustry style={{marginRight:4}} /> {say(selected.vendor)}</div>
              </div>
              <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: 22 }}></button>
            </div>

            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 13, color: '#9ca3af', marginBottom: 8 }}>{say("Update Status")}</div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.08)', fontSize: 12, color: '#6b7280', fontWeight: 700, textTransform: 'uppercase' }}>
                <span>{say("Description")}</span><span>{say("Part #")}</span><span>{say("Qty")}</span><span style={{ textAlign: 'right' }}>{say("Cost")}</span>
              </div>
              {selected.items?.map((item, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)', fontSize: 14 }}>
                  <span>{say(item.description)}</span>
                  <span style={{ color: '#9ca3af' }}>{say(item.partNumber)}</span>
                  <span>{say(item.qty)}</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '12px 16px', fontWeight: 700, fontSize: 16, color: '#22c55e' }}>
                {say("Total: $")}{(selected.total ?? 0).toFixed(2)}
              </div>
            </div>
            {selected.expectedDate && <div style={{ color: '#e5e7eb', fontSize: 14, marginBottom: 12 }}>{say("Expected delivery:")}{' '}{say(selected.expectedDate)}</div>}
            {selected.notes && <div style={{ color: '#9ca3af', fontSize: 13 }}><FaClipboardList style={{marginRight:4}} /> {say(selected.notes)}</div>}
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: '#1a1d27', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 16, padding: 32, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>{say("New Purchase Order")}</h2>
              <button onClick={() => setShowNew(false)} style={{ background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: 22 }}></button>
            </div>

            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Vendor *")}</label>
              <input value={form.vendor} onChange={e => setForm(f => ({ ...f, vendor: e.target.value }))}
                placeholder={say("Vendor name")} style={{ width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '10px 14px', color: '#e5e7eb', fontSize: 14, boxSizing: 'border-box' }} />
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Expected Delivery Date")}</label>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <label style={{ fontSize: 13, color: '#9ca3af', fontWeight: 700 }}>{say("Line Items")}</label>
                <button onClick={addItem} style={{ background: 'rgba(229,51,42,0.2)', color: '#e5332a', border: 'none', borderRadius: 6, padding: '4px 12px', cursor: 'pointer', fontSize: 13 }}>{say("+ Add Item")}</button>
              </div>
              {form.items.map((item, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 0.7fr 0.9fr auto', gap: 8, marginBottom: 8 }}>
                  <input value={item.description} onChange={e => updateItem(i, 'description', e.target.value)} placeholder={say("Description")}
                  ...
                  <input value={item.partNumber} onChange={e => updateItem(i, 'partNumber', e.target.value)} placeholder={say("Part #")}
                    style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, padding: '8px 12px', color: '#e5e7eb', fontSize: 13 }} />
                  <input type="number" value={item.qty} min={1} onChange={e => updateItem(i, 'qty', Number(e.target.value))} placeholder={say("Qty")}
                    style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, padding: '8px 12px', color: '#e5e7eb', fontSize: 13 }} />
                  <input type="number" value={item.unitCost} min={0} step={0.01} onChange={e => updateItem(i, 'unitCost', Number(e.target.value))} placeholder={say("Unit $")}
                    style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, padding: '8px 12px', color: '#e5e7eb', fontSize: 13 }} />
              ...
              <div style={{ textAlign: 'right', fontSize: 15, fontWeight: 700, color: '#22c55e', marginTop: 8 }}>{say("Total: $")}{total(form.items).toFixed(2)}</div>
            </div>

            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Notes")}</label>
              <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={3}
                placeholder={say("Optional notes or instructions...")}
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
              <button onClick={() => setShowNew(false)} style={{ background: 'rgba(255,255,255,0.08)', color: '#9ca3af', border: 'none', borderRadius: 8, padding: '10px 20px', cursor: 'pointer', fontSize: 14 }}>{say("Cancel")}</button>
              <button onClick={createPO} disabled={saving || !form.vendor.trim() || !form.items.some((item) => String(item.description || '').trim() && Number(item.qty) >= 1 && Number(item.unitCost) > 0)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer', opacity: !form.vendor.trim() || !form.items.some((item) => String(item.description || '').trim() && Number(item.qty) >= 1 && Number(item.unitCost) > 0) ? 0.5 : 1 }}>
                {saving ? say("Creating...") : say("Create Purchase Order")}
              </button>
            </div>
          </div>
        </div>
