  ...
  return (
    <div className="centered-app-page" style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ background: 'rgba(0,0,0,0.3)', padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700 }}><FaSearch style={{marginRight:4}} /> {say("DTC Code Lookup")}</h1>
        <p style={{ margin: '4px 0 0', color: '#9ca3af', fontSize: 14 }}>{say("Diagnose OBD-II fault codes with detailed repair information")}</p>
      </div>
      ...
      <div style={{ padding: 32, maxWidth: 720 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 32 }}>
          <input value={code} onChange={e => setCode(e.target.value.toUpperCase())} onKeyDown={e => e.key === 'Enter' && lookup()}
            placeholder={say("Enter DTC code (e.g. P0300, P0420, C1234...)")}
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 8 }}>{say("Common codes:")}</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[say("P0300"), say("P0420"), say("P0101"), say("P0171"), say("P0440"), say("P0128"), say("B0100"), say("C1234"), say("U0100"), say("P0562")].map(c => (
              <button key={c} onClick={() => { setCode(c); }} style={{ background: 'rgba(255,255,255,0.06)', color: '#9ca3af', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '5px 12px', fontSize: 13, fontFamily: 'monospace', cursor: 'pointer' }}>{say(c)}</button>
            ))}
          </div>
        </div>
        ...
        {error && (
          <div style={{ background: 'rgba(229,51,42,0.1)', border: '1px solid rgba(229,51,42,0.3)', borderRadius: 10, padding: 16, color: '#fca5a5', marginBottom: 20 }}>
            <FaExclamationTriangle style={{marginRight:4}} /> {say(error)}  {say("-  This code may be vehicle-specific or not in our database. Try Googling it or consult the vehicle service manual.")}{' '}</div>
        )}
        ...
        {result && (
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 24, marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 32, fontWeight: 800, fontFamily: 'monospace', letterSpacing: '0.1em' }}>{say(result.code)}</div>
                <div style={{ fontSize: 18, fontWeight: 700, marginTop: 4 }}>{say(result.description)}</div>
              </div>
            ...
            {result.systems?.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>{say("Affected Systems")}</div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {result.systems.map(s => <span key={s} style={{ background: 'rgba(96,165,250,0.15)', color: '#ff6b64', border: '1px solid rgba(96,165,250,0.3)', borderRadius: 6, padding: '3px 10px', fontSize: 13 }}>{say(s)}</span>)}
                </div>
              </div>
            )}
            ...
            {result.possibleCauses?.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>{say("Possible Causes")}</div>
                <ul style={{ margin: 0, padding: '0 0 0 20px', color: '#d1d5db', fontSize: 14, lineHeight: 1.8 }}>
                  {result.possibleCauses.map(c => <li key={c}>{say(c)}</li>)}
                </ul>
              </div>
            )}
            ...
            {result.commonFixes?.length > 0 && (
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>{say("Common Fixes")}</div>
                <ul style={{ margin: 0, padding: '0 0 0 20px', color: '#d1d5db', fontSize: 14, lineHeight: 1.8 }}>
                  {result.commonFixes.map(f => <li key={f}>{say(f)}</li>)}
                </ul>
              </div>
            )}
          </div>
        ...
        {history.length > 0 && (
          <div>
            <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 10 }}>{say("Recent lookups this session:")}</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {history.map(h => (
                <button key={h.code} onClick={() => { setCode(h.code); setResult(h); }}
                  ...
                  <span style={{ fontFamily: 'monospace', fontWeight: 700, color: '#e5e7eb' }}>{say(h.code)}</span>
