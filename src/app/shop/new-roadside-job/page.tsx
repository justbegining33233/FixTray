    ...
    <div style={{ minHeight: '100vh', background: 'transparent' }}>
      <div style={{ background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(229,51,42,0.3)', padding: '20px 32px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#e5e7eb', marginBottom: 8 }}><FaCar style={{ marginRight: 4 }} /> {say("New Roadside Job")}</h1>
          <p style={{ fontSize: 14, color: '#9aa3b2' }}>{say("Create a new roadside assistance work order")}</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 32 }}>
        <form onSubmit={handleSubmit} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 32 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
            <div>
              <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Customer Name")}</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Customer Phone")}</label>
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Location (Address or GPS)")}</label>
            <input type="text" required value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} placeholder={say("Street address or coordinates")} style={{ width: '100%', padding: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, color: '#e5e7eb', fontSize: 14 }} />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 24 }}>
            <div>
              <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Vehicle Year")}</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Make")}</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Model")}</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
            <div>
              <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Service Type")}</label>
              <select
              ...
              >
                {servicesLoading && <option value="">{say("Loading services...")}</option>}
                {!servicesLoading && serviceOptions.length === 0 && <option value="">{say("No shop services configured")}</option>}
              ...
              {!servicesLoading && serviceOptions.length === 0 && (
                <p style={{ marginTop: 8, fontSize: 12, color: '#f59e0b' }}>
                  {say("Add services in Shop Settings before creating roadside work orders.")}{' '}</p>
              )}
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Urgency Level")}</label>
              <select value={formData.urgency} onChange={(e) => setFormData({ ...formData, urgency: e.target.value })} style={{ width: '100%', padding: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, color: '#e5e7eb', fontSize: 14, cursor: 'pointer' }}>
                <option value="low">{say("Low")}</option>
                <option value="normal">{say("Normal")}</option>
                <option value="high">{say("High")}</option>
                <option value="emergency">{say("Emergency")}</option>
              </select>
            </div>
          </div>

          <div style={{ marginBottom: 24 }}>
            <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Issue Description")}</label>
          ...
          <div style={{ marginBottom: 32 }}>
            <label style={{ display: 'block', fontSize: 14, color: '#e5e7eb', marginBottom: 8, fontWeight: 600 }}>{say("Additional Notes")}</label>
            <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={3} placeholder={say("Optional notes...")} style={{ width: '100%', padding: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, color: '#e5e7eb', fontSize: 14, resize: 'vertical' }} />
          </div>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button type="submit" disabled={servicesLoading || serviceOptions.length === 0 || submitting} style={{ flex: 1, padding: '14px', background: '#e5332a', color: 'white', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', opacity: servicesLoading || serviceOptions.length === 0 || submitting ? 0.6 : 1 }}>
              {submitting ? say("Creating…") : say("Create Roadside Job")}
            </button>
