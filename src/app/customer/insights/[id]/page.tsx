    ...
    <div style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb' }}>
      <div style={{ maxWidth: 860, margin: '0 auto', padding: 32 }}>
        <Link href="/customer/insights" style={{ color: '#e5332a', textDecoration: 'none', fontWeight: 700 }}>
          <FaArrowLeft style={{ marginRight: 6 }} /> Back to Insights
        </Link>
        {loading ? <p style={{ marginTop: 32, color: '#9aa3b2' }}>Loading insight...</p> : null}
        {!loading && missing ? (
          <div style={{ marginTop: 32, padding: 24, borderRadius: 12, background: 'rgba(0,0,0,0.3)' }}>
            <h1 style={{ fontSize: 24 }}>Insight not found</h1>
            <p style={{ color: '#9aa3b2' }}>That insight is not available for this account.</p>
          </div>
        ) : null}
        {insight ? (
          <div style={{ marginTop: 24 }}>
            <h1 style={{ fontSize: 32, marginBottom: 8 }}>{insight.metric}</h1>
            <div style={{ fontSize: 36, fontWeight: 800, color: insight.color }}>{insight.value}</div>
            <p style={{ color: '#9aa3b2' }}>{insight.trend}</p>
            <p style={{ lineHeight: 1.6, maxWidth: 640 }}>{insight.description}</p>
            {insightId === 'loyalty-points' ? (
              <p style={{ marginTop: 16 }}>
                Balance used by Rewards: <strong>{loyaltyPoints ?? insight.value}</strong>.{' '}
                <Link href="/customer/rewards" style={{ color: '#e5332a' }}>Open rewards</Link>
              </p>
            ) : null}
            {showOrders ? (
              <div style={{ marginTop: 24, display: 'grid', gap: 12 }}>
                {rows.length === 0 ? <p style={{ color: '#9aa3b2' }}>No completed services in this window.</p> : null}
                {rows.map((row) => (
                  <div key={row.id} style={{ padding: 16, borderRadius: 12, background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    ...
                    <div style={{ color: '#9aa3b2', fontSize: 14 }}>{row.shop} · {new Date(row.date).toLocaleDateString()} · ${Number(row.amount).toFixed(2)}</div>
                  </div>
                ))}
              </div>
            ) : null}
