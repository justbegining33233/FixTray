  ...
  return (
    <div style={{ minHeight: '100vh', background: 'transparent' }}>
        ...
        <div style={{ maxWidth: 1600, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            <div>
              ...
              <p style={{ fontSize: 14, color: '#9aa3b2' }}>{say("Live data from your shop's work orders and payments")}</p>
            </div>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} style={{ padding: '10px 16px', background: 'rgba(0,0,0,0.4)', color: '#e5e7eb', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                {yearOptions.map(y => <option key={y} value={y}>{say(y)}</option>)}
              </select>
              <select value={selectedMonth} onChange={(e) => setSelectedMonth(Number(e.target.value))} style={{ padding: '10px 16px', background: 'rgba(0,0,0,0.4)', color: '#e5e7eb', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                {MONTH_NAMES.map((m, i) => <option key={i} value={i + 1}>{say(m)}</option>)}
              </select>
              <button onClick={handleExport} disabled={!report} style={{ padding: '10px 20px', background: '#22c55e', color: 'white', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: report ? 'pointer' : 'not-allowed', opacity: report ? 1 : 0.5 }}>
                <FaDownload style={{marginRight:4}} /> {say("Export CSV")}{' '}</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1600, margin: '0 auto', padding: 32 }}>
        {loading && (
          <div style={{ textAlign: 'center', color: '#9aa3b2', fontSize: 16, padding: 48 }}>{say("Loading report data...")}</div>
        )}

        {!loading && !report && (
          <div style={{ textAlign: 'center', color: '#9aa3b2', fontSize: 16, padding: 48 }}>{say("No data available for")}{' '}{MONTH_NAMES[selectedMonth - 1]} {say(selectedYear)}.</div>
        )}
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24, marginBottom: 24 }}>
              ...
              <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 24 }}>
                <h2 style={{ fontSize: 20, fontWeight: 700, color: '#e5e7eb', marginBottom: 20 }}>{say("Revenue Trend  -")}{' '}{say(selectedYear)}</h2>
                  ...
                  {report.revenueByMonth.map((mo, idx) => {
                    ...
                    return (
                      <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                        <div style={{ width: 40, fontSize: 12, color: isSelected ? '#f59e0b' : '#9aa3b2', fontWeight: isSelected ? 700 : 500 }}>{say(mo.month)}</div>
                        ...
                        <div style={{ width: 64, fontSize: 12, color: '#9aa3b2', textAlign: 'right' }}>{say(mo.jobs)} jobs</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 24 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {report.topServices.length === 0 && (
                    <div style={{ color: '#6b7280', fontSize: 14 }}>{say("No service data for this period.")}</div>
                  )}
                  {report.topServices.map((svc, idx) => (
                    <div key={idx} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: 14 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#e5e7eb' }}>{say(svc.service)}</div>
                        <div style={{ fontSize: 15, fontWeight: 700, color: '#22c55e' }}>${svc.revenue.toLocaleString()}</div>
                      </div>
                      <div style={{ fontSize: 12, color: '#9aa3b2' }}>{say(svc.jobs)} jobs</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Tech Performance */}
            <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 24 }}>
              <h2 style={{ fontSize: 20, fontWeight: 700, color: '#e5e7eb', marginBottom: 20 }}>{say("Technician Performance  -")}{' '}{MONTH_NAMES[selectedMonth - 1]} {say(selectedYear)}</h2>
              {report.techPerformance.length === 0 ? (
                <div style={{ color: '#6b7280', fontSize: 14 }}>{say("No technician data for this period.")}</div>
              ) : (
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 8px' }}>
                    <thead>
                      <tr style={{ textAlign: 'left' }}>
                        <th style={{ padding: '8px 16px', color: '#9aa3b2', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' }}>{say("Technician")}</th>
                        <th style={{ padding: '8px 16px', color: '#9aa3b2', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' }}>{say("Jobs")}</th>
                        <th style={{ padding: '8px 16px', color: '#9aa3b2', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' }}>{say("Revenue")}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {report.techPerformance.map((tech, idx) => (
                        <tr key={idx} style={{ background: 'rgba(255,255,255,0.04)' }}>
                          <td style={{ padding: '12px 16px', color: '#e5e7eb', fontWeight: 700 }}>{say(tech.name)}</td>
                          <td style={{ padding: '12px 16px', color: '#9aa3b2' }}>{say(tech.jobs)}</td>
