    ...
    <div style={{minHeight:'100vh', background: 'transparent'}}>
      <div style={{background:'rgba(0,0,0,0.3)', borderBottom:'1px solid rgba(245,158,11,0.3)', padding:'20px 32px'}}>
        <div style={{maxWidth:1400, margin:'0 auto'}}>
          ...
          <p style={{fontSize:14, color:'#9aa3b2'}}>{say("Revenue, payouts, and financial analytics")}</p>
        </div>
      </div>
      ...
      <div style={{maxWidth:1400, margin:'0 auto', padding:32}}>
        {loading ? (
          ...
          <div style={{background:'rgba(34,197,94,0.1)', border:'1px solid rgba(34,197,94,0.3)', borderRadius:12, padding:20}}>
            ...
            <div style={{fontSize:11, color:'#9aa3b2', marginTop:4}}>{say("All time")}</div>
          </div>
          <div style={{background:'rgba(229,51,42,0.1)', border:'1px solid rgba(229,51,42,0.3)', borderRadius:12, padding:20}}>
            <div style={{fontSize:13, color:'#9aa3b2', marginBottom:8}}>{say("Total Payouts")}</div>
            <div style={{fontSize:28, fontWeight:700, color:'#e5332a'}}>{say(stats.totalPayouts)}</div>
            <div style={{fontSize:11, color:'#9aa3b2', marginTop:4}}>{say("Paid to shops")}</div>
          </div>
          <div style={{background:'rgba(245,158,11,0.1)', border:'1px solid rgba(245,158,11,0.3)', borderRadius:12, padding:20}}>
            <div style={{fontSize:13, color:'#9aa3b2', marginBottom:8}}>{say("Platform Fees")}</div>
            <div style={{fontSize:28, fontWeight:700, color:'#f59e0b'}}>{say(stats.platformFees)}</div>
            <div style={{fontSize:11, color:'#9aa3b2', marginTop:4}}>{say("Flat platform fee model")}</div>
          </div>
          <div style={{background:'rgba(229,51,42,0.1)', border:'1px solid rgba(229,51,42,0.3)', borderRadius:12, padding:20}}>
            <div style={{fontSize:13, color:'#9aa3b2', marginBottom:8}}>{say("Pending Payouts")}</div>
            <div style={{fontSize:28, fontWeight:700, color:'#e5332a'}}>{say(stats.pendingPayouts)}</div>
            <div style={{fontSize:11, color:'#9aa3b2', marginTop:4}}>{say("Awaiting processing")}</div>
          </div>
        </div>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginBottom:24}}>
          ...
          <div style={{background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24}}>
            <h2 style={{fontSize:20, fontWeight:700, color:'#e5e7eb', marginBottom:20}}>{say("Monthly Revenue")}</h2>
                ...
                monthlyData.map((data, idx) => (
                  <div key={idx} style={{background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, padding:16}}>
                    ...
                    <div style={{display:'flex', gap:16, fontSize:13, color:'#9aa3b2'}}>
                      <span>{say("Payouts:")}{' '}{say(data.payouts)}</span>
                      <span>{say("Fees:")}{' '}{say(data.fees)}</span>
                    </div>
                  </div>
                ))
              )}
          ...
          {/* Top Earning Shops */}
          <div style={{background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24}}>
            <h2 style={{fontSize:20, fontWeight:700, color:'#e5e7eb', marginBottom:20}}>{say("Top Earning Shops")}</h2>
            <div style={{display:'flex', flexDirection:'column', gap:12}}>
              {topEarningShops.length === 0 ? (
                <div style={{textAlign:'center', padding:32, color:'#9aa3b2'}}>
                  <div style={{fontSize:24, marginBottom:8}}><FaStore style={{marginRight:4}} /></div>
                  <div>{say("No shop data available")}</div>
                </div>
              ) : (
                topEarningShops.map((shop, idx) => (
                  <div key={idx} style={{background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, padding:16}}>
                    <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:8}}>
                      ...
                      <span style={{fontSize:15, fontWeight:700, color:'#e5e7eb'}}>{say(shop.name)}</span>
                    </div>
                    <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, fontSize:13}}>
                      <div>
                        <div style={{color:'#6b7280', fontSize:11}}>{say("Revenue")}</div>
                        <div style={{color:'#22c55e', fontWeight:700}}>{say(shop.revenue)}</div>
                      </div>
                      <div>
                        <div style={{color:'#6b7280', fontSize:11}}>{say("Fees")}</div>
                        <div style={{color:'#f59e0b', fontWeight:700}}>{say(shop.fees)}</div>
                      </div>
                      <div>
                        <div style={{color:'#6b7280', fontSize:11}}>{say("Payout")}</div>
                        <div style={{color:'#e5332a', fontWeight:700}}>{say(shop.payout)}</div>
                      </div>
                    </div>
                  </div>
                ))
        ...
        <div style={{background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(250px, 1fr))', gap:16}}>
            <div style={{background:'rgba(255,255,255,0.05)', borderRadius:8, padding:16}}>
              <div style={{fontSize:13, color:'#9aa3b2', marginBottom:4}}>{say("Total Transactions")}</div>
              <div style={{fontSize:24, fontWeight:700, color:'#e5e7eb'}}>{stats.transactionCount.toLocaleString()}</div>
            </div>
            <div style={{background:'rgba(255,255,255,0.05)', borderRadius:8, padding:16}}>
              <div style={{fontSize:13, color:'#9aa3b2', marginBottom:4}}>{say("Average Transaction")}</div>
              <div style={{fontSize:24, fontWeight:700, color:'#e5e7eb'}}>{say(stats.averageTransaction)}</div>
            </div>
            <div style={{background:'rgba(255,255,255,0.05)', borderRadius:8, padding:16}}>
              <div style={{fontSize:13, color:'#9aa3b2', marginBottom:4}}>{say("Pricing Model")}</div>
              <div style={{fontSize:24, fontWeight:700, color:'#f59e0b'}}>{say("Flat Fee")}</div>
            </div>
          </div>
        </div>
          </>
