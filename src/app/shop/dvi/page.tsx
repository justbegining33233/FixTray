  ...
  return (
    <div className="centered-app-page" style={{ minHeight: '100vh', background: 'transparent', color: '#e5e7eb', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ background: 'rgba(0,0,0,0.3)', padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700 }}><FaSearch style={{marginRight:4}} /> {say("Digital Vehicle Inspections")}</h1>
          <p style={{ margin: '4px 0 0', color: '#9ca3af', fontSize: 14 }}>{say("Multi-point inspections sent to customers for approval  -  average 35% upsell lift")}</p>
        </div>
        <button onClick={() => setShowNew(true)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{say("+ New DVI")}</button>
      </div>

      <div style={{ padding: 32, display: 'grid', gridTemplateColumns: selected ? '340px 1fr' : '1fr', gap: 24 }}>
        {/* List */}
        <div>
          {loading ? <div style={{ color: '#6b7280', padding: 32 }}>{say("Loading...")}</div> :
            inspections.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 80 }}>
                <div style={{ fontSize: 64 }}><FaSearch style={{marginRight:4}} /></div>
                <div style={{ fontSize: 18, fontWeight: 600, margin: '16px 0 8px' }}>{say("No inspections yet")}</div>
                <div style={{ color: '#9ca3af', marginBottom: 24 }}>{say("Create your first DVI to start upselling approved work")}</div>
                <button onClick={() => setShowNew(true)} style={{ background: '#e5332a', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 28px', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>{say("+ Start First DVI")}</button>
              </div>
            ) : (
                ...
                return (
                  <div key={insp.id} onClick={() => setSelected(insp)}
                    ...
                    <div style={{ display: 'flex', gap: 12, fontSize: 12 }}>
                      ...
                      {yellows > 0 && <span style={{ color: '#f59e0b' }}> {say(yellows)} advisory</span>}
                      {insp.customerApproved && <span style={{ color: '#22c55e' }}><FaCheckCircle style={{marginRight:4}} /> {say("Approved")}</span>}
                    </div>
                  </div>
                );
              })
        ...
        {selected && (
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 20, overflowY: 'auto', maxHeight: '80vh' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 18 }}>{selected.vehicleDesc || say("Vehicle Inspection")}</h3>
                ...
                {selected.items.filter(i => i.category === cat).map(item => (
                  <div key={item.id} style={{ background: conditionBg[item.condition], border: `1px solid ${conditionColor[item.condition]}40`, borderRadius: 8, padding: '10px 14px', marginBottom: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      {item.notes && <div style={{ fontSize: 12, color: '#9ca3af' }}>{say(item.notes)}</div>}
                      {item.estimatedCost && <div style={{ fontSize: 12, color: '#f59e0b' }}>${say(item.estimatedCost)}</div>}
                    </div>
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: '#1f2937', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 28, width: 420, maxWidth: '90%' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 18 }}>{say("Start New DVI")}</h3>
            <p style={{ color: '#9ca3af', fontSize: 13, marginTop: 0, marginBottom: 20 }}>{say("A 19-point inspection template will be created. You can adjust each item&apos;s condition () after creation.")}</p>
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 13, color: '#9ca3af', display: 'block', marginBottom: 6 }}>{say("Work Order (from bays/waiting)")}</label>
              <select
              ...
              >
                <option value="">{say("Select work order...")}</option>
            ...
            {[["vehicleDesc", say("Vehicle (Year/Make/Model)")], ['mileage', say("Current Mileage")], ['notes', say("Notes (optional)")]].map(([k, label]) => (
