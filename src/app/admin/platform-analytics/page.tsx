...
return (<div className="h-64 flex items-center justify-center text-gray-400">{say("Loading chart...")}</div>); }
});
    ...
    <div style={{minHeight:'100vh', background: 'transparent'}}>
      <div style={{background:'rgba(0,0,0,0.3)', borderBottom:'1px solid rgba(245,158,11,0.3)', padding:'20px 32px'}}>
        <div style={{maxWidth:1400, margin:'0 auto'}}>
          ...
          <h1 style={{fontSize:28, fontWeight:700, color:'#e5e7eb', marginBottom:8}}><FaChartBar style={{marginRight:4}} /> {say("Platform Analytics")}</h1>
          <p style={{fontSize:14, color:'#9aa3b2'}}>{say("Detailed analytics and performance metrics")}</p>
        </div>
      </div>
      ...
      <div style={{maxWidth:1400, margin:'0 auto', padding:32}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'2fr 1fr', gap:24}}>
          ...
          <div style={{background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24}}>
            <h2 style={{fontSize:20, fontWeight:700, color:'#e5e7eb', marginBottom:20}}>{say("Revenue Trends")}</h2>
<DynamicRevenueChart data={data.revenue} />
        </div>

          {/* Top Shops */}
          <div style={{background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24}}>
            <h2 style={{fontSize:20, fontWeight:700, color:'#e5e7eb', marginBottom:20}}>{say("Status Overview")}</h2>
            <div style={{display:'flex', flexDirection:'column', gap:12}}>
              {fetchError && <div style={{color:'#f87171', fontSize:13}}>{say("Could not load live data:")}{' '}{say(fetchError)}</div>}
              {[
                { label: say("Pending"), val: data.pendingWorkOrders, color: '#f59e0b' },
                { label: say("In Progress"), val: data.inProgressWorkOrders, color: '#e5332a' },
                { label: say("Completed"), val: data.completedWorkOrders, color: '#22c55e' },
              ].map(row => (
                <div key={row.label} style={{padding:12, background:'rgba(255,255,255,0.05)', borderRadius:8, display:'flex', justifyContent:'space-between'}}>
                  <div style={{fontSize:14, fontWeight:600, color:'#e5e7eb'}}>{say(row.label)}</div>
        ...
        <div style={{marginTop:24, background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24}}>
          <h2 style={{fontSize:20, fontWeight:700, color:'#e5e7eb', marginBottom:20}}>{say("Completion Times")}</h2>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginTop:24}}>
          ...
          {/* Status Distribution */}
          <div style={{background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24}}>
            <h2 style={{fontSize:20, fontWeight:700, color:'#e5e7eb', marginBottom:20}}>{say("Work Order Status")}</h2>
        ...
        {/* Monthly Trends */}
        <div style={{marginTop:24, background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:24}}>
          <h2 style={{fontSize:20, fontWeight:700, color:'#e5e7eb', marginBottom:20}}>{say("Monthly Trends")}</h2>
