      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px' }}>
        <div style={{ background: 'rgba(229,51,42,0.1)', border: '1px solid rgba(229,51,42,0.3)', borderRadius: 10, padding: '16px 20px', marginBottom: 28, display: 'flex', gap: 12 }}>
          ...
          <div style={{ fontSize: 13, color: '#9aa3b2', lineHeight: 1.6 }}>
            {say("Backups export a JSON snapshot of all platform data. Sensitive fields (passwords, payment tokens) are excluded. Store backups securely.")}{' '}</div>
        </div>
        ...
        <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, overflow: 'hidden', marginBottom: 24 }}>
          <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontWeight: 700, color: '#e5e7eb', fontSize: 16 }}>{say("Download Full Backup")}</div>
              <div style={{ fontSize: 13, color: '#9aa3b2', marginTop: 3 }}>{say("Exports platform data as a JSON file")}</div>
            </div>
          ...
          <div style={{ padding: '24px' }}>
            {lastBackup && (
              <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 8, padding: '10px 16px', marginBottom: 20, fontSize: 13, color: '#86efac' }}>
                <FaCheckCircle style={{marginRight:4}} /> {say("Last backup:")}{' '}{say(lastBackup)}
              </div>
            )}
            <button onClick={handleDownload} disabled={downloading}
              ...
              {downloading ? <><FaHourglassHalf style={{marginRight:4}} /> {say("Preparing...")}</> : <><FaArrowDown style={{marginRight:4}} /> {say("Download Backup (JSON)")}</>}
            </button>
          </div>
        </div>

        <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 24, marginBottom: 24 }}>
          <div style={{ fontWeight: 700, color: '#e5e7eb', fontSize: 15, marginBottom: 16 }}>{say("What's included")}</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
            {[say("Shops"), say("Customers"), say("Technicians"), say("Work Orders"), say("Appointments"), say("Reviews"), say("Invoices"), say("Notifications")].map(item => (
              <div key={item} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#d1d5db', display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ color: '#22c55e' }}><FaCheck style={{marginRight:4}} /></span> {say(item)}
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: 10, padding: '16px 20px', display: 'flex', gap: 12 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, color: '#fbbf24', marginBottom: 4 }}>{say("Restore Process")}</div>
            <div style={{ fontSize: 13, color: '#9aa3b2', lineHeight: 1.6 }}>
              {say("Restore requires importing the JSON file via Prisma CLI or a migration script. Automated restore UI is planned for a future release.")}{' '}</div>
          </div>
        </div>
      </div>
    </div>
