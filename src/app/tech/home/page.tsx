    ...
    >
          ...
          <div>
            ...
            <div style={{marginTop:0, marginBottom:24}}>
              ...
              {activeTab === 'job-creation' && (
                <div style={{display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(300px, 1fr))', gap: isMobile ? 12 : 20, marginBottom:32}}>
              ...
              <div style={{flex:1, display:'flex', flexDirection:'column'}}>
                {shopCoords ? (
                  <div style={{display:'flex', flexDirection: isMobile ? 'column' : 'row', flex:1}}>
                    ...
                    {/* Right-side menu: 3/8 - Road Call / Parts / User Location */}
      ...
      {homeMsg && (
        <div style={{position:'fixed',bottom:24,right:24,background:homeMsg.type==='success'?'#dcfce7':'#fde8e8',color:homeMsg.type==='success'?'#166534':'#991b1b',borderRadius:10,padding:'12px 20px',zIndex:9999,fontSize:14,fontWeight:600,boxShadow:'0 4px 12px rgba(0,0,0,0.3)'}}>
          {say(homeMsg.text)}
          <button aria-label={say("Dismiss")} onClick={()=>setHomeMsg(null)} style={{marginLeft:12,background:'none',border:'none',cursor:'pointer',fontSize:16,color:'inherit'}}>×</button>
        </div>
      )}
    </MobileLayout>
  );
